...
from service.app.users.mixins import SignupLoginMixin
from service.app.users.models import AppUser, PasswordReset

from service.app.utils.auth import auth_required, admin_required, generate_token
from service.app.utils.errors import EMAIL_IN_USE, CODE_NOT_VALID, BAD_CREDENTIALS

from service.app import db, bcrypt
...
class PasswordResetConfirmAPI(restful.Resource):
    ...
    def post(self):
        ...
        password_reset = db.session.query(PasswordReset).filter(
            PasswordReset.code == args['code']).filter(
            PasswordReset.date > datetime.now()).first()
...
class ServiceLifecycleAPI(restful.Resource):
